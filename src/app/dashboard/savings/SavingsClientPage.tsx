    ...
    <div className="space-y-10 animate-fade-in text-left">
      <div className="flex flex-col lg:flex-row justify-between items-center gap-6">
        ...
        <div className="flex flex-col items-end gap-3">
          <div className="flex flex-wrap items-center justify-center gap-3 bg-white dark:bg-[#1E1E2D] p-1.5 rounded-[24px] border border-gray-100 dark:border-gray-800 shadow-sm">
            ...
            <div className="flex items-center gap-1 pr-2">
              <button onClick={() => changePeriod(-1)} className="p-2 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-xl transition-all active:scale-90"><ChevronLeft className="w-4 h-4 text-gray-600" /></button>
              <div className="px-2 min-w-[100px] text-center">
                <span className="font-black text-[10px] text-gray-900 dark:text-white tracking-tight uppercase">
                  {viewMode === "monthly" ? format(currentDate, "MMM yyyy", { locale: id }) : format(currentDate, "yyyy", { locale: id })}
                </span>
              </div>
              <button onClick={() => changePeriod(1)} className="p-2 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-xl transition-all active:scale-90"><ChevronRight className="w-4 h-4 text-gray-600" /></button>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
              <button 
                  ...
                  className="bg-white dark:bg-[#1E1E2D] text-amber-600 border border-amber-200 dark:border-amber-800 px-6 py-4 rounded-[24px] text-sm font-black shadow-sm hover:bg-amber-50 dark:hover:bg-amber-900/10 transition-all flex items-center gap-2 active:scale-95"
              >
              ...
              <button 
                  ...
                  className="bg-white dark:bg-[#1E1E2D] text-blue-600 border border-blue-200 dark:border-blue-800 px-6 py-4 rounded-[24px] text-sm font-black shadow-sm hover:bg-blue-50 dark:hover:bg-blue-900/10 transition-all flex items-center gap-2 active:scale-95"
              >
              ...
              <button 
                  ...
                  className="bg-gradient-to-r from-amber-500 to-yellow-500 text-white px-6 py-4 rounded-[24px] text-sm font-black shadow-xl shadow-amber-500/20 hover:opacity-90 transition-all flex items-center gap-2 active:scale-95"
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
           ...
           <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:scale-110 transition-transform duration-500">
            ...
            <div className="relative z-10">
               <div className="flex items-center gap-3 mb-2">
                 ...
                 <p className="text-amber-100 text-[11px] font-black uppercase tracking-[0.2em] opacity-90">Total Tabungan (Kumulatif)</p>
               </div>
               <h3 className="text-5xl font-black tracking-tighter">Rp {totalSavingsPool.toLocaleString("id-ID")}</h3>
               <div className="mt-2 flex items-center gap-2">
                 <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full backdrop-blur-md border border-white/10 ${periodStats.totalActivity >= 0 ? 'bg-emerald-500/30 text-emerald-100' : 'bg-rose-500/30 text-rose-100'}`}>
                    {periodStats.totalActivity >= 0 ? '+' : ''} Rp {periodStats.totalActivity.toLocaleString("id-ID")} ({viewMode === 'monthly' ? 'Bulan Ini' : 'Tahun Ini'})
                 </span>
               </div>
            </div>
           <div className="mt-12 relative z-10">
              <div className="flex items-center gap-2 text-amber-100/80 text-[10px] font-black uppercase tracking-wider bg-black/10 w-fit px-5 py-2.5 rounded-full backdrop-blur-md border border-white/5">
                 <ArrowRightLeft className="w-4 h-4 text-amber-300" />
                 Dana terpisah dari saldo operasional
              </div>
           </div>
        </GlassCard>
        ...
        <div className="lg:col-span-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Tabungan Umum Card */}
                <GlassCard className="p-8 border-dashed border-2 border-amber-200 dark:border-amber-900/50 hover:border-amber-500/50 transition-all group relative overflow-hidden bg-amber-50/10 dark:bg-amber-900/5">
                    <div className="flex justify-between items-start mb-6">
                        <div className="space-y-1">
                            <h4 className="font-black text-xl text-gray-900 dark:text-white group-hover:text-amber-500 transition-colors">Tabungan Umum</h4>
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Dana Belum Dialokasikan</p>
                        </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-end">
                            <div className="space-y-1">
                                <span className="text-xs font-black text-gray-500 uppercase tracking-widest block">Total Tersedia</span>
                                <div className="flex items-center gap-2">
                                    <span className="text-2xl font-black text-amber-600 tracking-tight">Rp {unallocatedSavings.toLocaleString("id-ID")}</span>
                                    <span className={`text-[8px] font-black px-2 py-0.5 rounded-md ${periodStats.unallocatedActivity >= 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                                        {periodStats.unallocatedActivity >= 0 ? '+' : ''} {periodStats.unallocatedActivity.toLocaleString("id-ID")}
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-700/50 flex items-center justify-between gap-4">
                            <p className="text-[10px] font-bold text-gray-400 italic flex-1">
                                Dana ini dapat dialokasikan ke goal tertentu kapan saja.
                            </p>
                            <button 
                                onClick={() => {
                                ...
                                className="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-white text-[9px] font-black uppercase tracking-widest rounded-lg transition-colors shrink-0"
                            >
                                Alokasikan
                            </button>
                        </div>
                        ...
                        <GlassCard key={goal.id} className={`p-8 hover:border-amber-500/50 transition-all group relative overflow-hidden ${isMain ? 'border-amber-500 ring-2 ring-amber-500/20 shadow-xl shadow-amber-500/10' : ''}`}>
                            ...
                            <div className="flex justify-between items-start mb-6">
                                <div className="space-y-1">
                                    ...
                                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Target: Rp {targetAmount.toLocaleString("id-ID")}</p>
                                </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex justify-between items-center text-[10px] font-bold text-gray-400 uppercase tracking-tighter">
                                    <span>Rp {goalBalance.toLocaleString("id-ID")}</span>
                ...
                {goals.length === 0 && (
                    <div className="flex flex-col items-center justify-center p-10 border-2 border-dashed border-gray-100 dark:border-gray-800 rounded-[32px] bg-gray-50/50 dark:bg-gray-900/50">
                        <Target className="w-8 h-8 text-gray-300 mb-3" />
                        <p className="text-xs font-black text-gray-400 uppercase tracking-widest">Belum ada Goal Spesifik</p>
                    </div>
                )}
            </div>
        </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-100/70 dark:bg-gray-800/50">
                                ...
                                <th className="px-8 py-5 text-[11px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-800">Jenis</th>
                                <th className="px-8 py-5 text-[11px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-800">Goal</th>
                        ...
                        <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                            {paginatedHistory.map((item) => (
                                <tr key={item.id} className="hover:bg-amber-50/30 dark:hover:bg-amber-900/5 transition-colors group">
                                    ...
                                    <td className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-center">
                                        <span className="inline-flex items-center px-3 py-1.5 rounded-xl bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 border border-gray-200/50 dark:border-gray-700/50">
                                            {item.goalName}
                                        </span>
                                    </td>
                                    ...
                                    <td className={`px-8 py-5 text-sm font-black text-center ${
